...
class particle(object):
	...
	def setUpBranches(self, wrappedOutputTree, prefix="g"):
		if self.branches_setup:
			return
		
		length_name = "{}".format(prefix+"n"+self.particleType)
		wrappedOutputTree.branch(length_name,"I")
		for bName, bType in self.branch_names.iteritems():
			wrappedOutputTree.branch("{}_{}".format(prefix+self.particleType, bName), bType, lenVar=length_name)
	...
	def fillBranches(self,wrappedOutputTree,prefix="g"):
		wrappedOutputTree.fillBranch("{}".format(prefix+"n"+self.particleType),len(self.collection))
		for bName in self.branch_names.keys():
			wrappedOutputTree.fillBranch("{}_{}".format(prefix+self.particleType, bName), self.get_attributes(bName))

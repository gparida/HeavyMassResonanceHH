...
class ChannelCamilla(Module):
	...
	def analyze(self, event): 
		...
		self.Tau.apply_cut(lambda x: (x.pt > 20) and (abs(x.eta) < 2.3) and (x.idDeepTau2017v2p1VSjet & 1 == 1) and ((x.decayMode == 0) or (x.decayMode == 1) or (x.decayMode == 2) or (x.decayMode == 7) or (x.decayMode == 10) or (x.decayMode == 11))) #VVVLoose
		...
		self.boostedTau.apply_cut(lambda x: (x.pt > 40) and (abs(x.eta) < 2.3) and (x.idMVAnewDM2017v2 & 1 == 1) and ((x.decayMode == 0) or (x.decayMode == 1) or (x.decayMode == 2) or (x.decayMode == 7) or (x.decayMode == 10) or (x.decayMode == 11))) # VVLoose
...
###################CutBased ID Check ##################################################################
		...
		self.LooseCutBasedElectron.apply_cut(lambda x: x.cutBased>=2 and (x.pt > 10))
...
def call_postpoc(files):
		...
		try:
			if filename == "Data":
				...
				p = PostProcessor(outputDir,[files], cut=cuts, branchsel=outputbranches,modules=[letsSortChannels(),tauOdering(),tauOdering2(),visibleM(),visibleM2(),mttBranches(),genMatch(),genMeasurementRadion()], postfix=post,noOut=False,outputbranchsel=outputbranches)
			else:

...
def call_postpoc(files):
		...
		try:
			if filename == "Data":
				p = PostProcessor(outputDir,[files], branchsel=outputbranches,modules=[letsSortChannels(),tauOdering(),tauOdering2(),visibleM(),visibleM2(),mttBranches()], postfix=post,noOut=False,outputbranchsel=outputbranches)
			else:
				p = PostProcessor(outputDir,[files], branchsel=outputbranches,modules=[letsSortChannels(),tauOdering(),tauOdering2(),visibleM(),visibleM2(),mttBranches()], postfix=post,noOut=False,outputbranchsel=outputbranches)
		except SystemError:

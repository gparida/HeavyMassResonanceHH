...
class ChannelCamilla(Module):
	...
	def setup_branches(self):
		...
		########Nov14_Added_for gen level channel sorting#########
		if search("Radion", self.filename):
	...
	def analyze(self, event): 
		...
		#if search("Radion", self.filename):
		#	decay,tauIndices,genlep, extraFV = classifyTauDecayMode(event._tree)
		#	if decay == "tt":
		...
		#	elif ((decay == None) or (decay == "one") or (decay == "zero") or (decay == "moreh") or (decay == "more")):
		...
		self.boostedTau.apply_cut(lambda x: (x.pt > 40) and (abs(x.eta) < 2.3) and (x.idMVAnewDM2017v2 & 1 == 1)) # VVLoose
